...
public class CateAction extends BaseAction {
	...
	public String getAllCate() {
		...
		if ((Integer.parseInt(this.number) + 1) == 1) {
			...
			buffer.append(
					"<a href=\"cate/getAllCate.action?number=" + (Integer.parseInt(this.number) - 1) + "\">上一页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if (this.maxPage <= (Integer.parseInt(this.number) + 1)) {
			...
			buffer.append(
					"<a href=\"cate/getAllCate.action?number=" + (Integer.parseInt(this.number) + 1) + "\">下一页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if (this.maxPage <= (Integer.parseInt(this.number) + 1)) {
			buffer.append("尾页");
		} else {
			buffer.append("<a href=\"cate/getAllCate.action?number=" + (this.maxPage - 1) + "\">尾页</a>");
		}
