...
public class ActivitiesAction extends BaseAction {
	...
	public String addActivities() {
		this.activities.setAddtime(VeDate.getStringDateShort());
		this.activities.setHits("0");
		this.activities.setNum("0");
		this.activitiesBusiness.save(this.activities);
		return SUCCESS;
	}
	...
	public String deleteActivities() {
		try {
			this.activitiesBusiness.delete(activities);
		} catch (Exception e) {
	...
	public String deleteActivitiesByIds() {
		try {
			for (String id : this.ids) {
				this.activitiesBusiness.delete(id);
			}
		} catch (Exception e) {
			this.map.put("msg", "存在关联项 不能删除");
		}
	...
	public String getAllActivities() {
		this.list = new ArrayList<Activities>();
		List<Activities> tempList = new ArrayList<Activities>();
		tempList = this.activitiesBusiness.show();
		...
		for (int i = start; i < over; i++) {
			Activities u = (Activities) tempList.get(i);
			this.list.add(u);
		}
		...
		if ((Integer.parseInt(this.number) + 1) == 1) {
			...
			buffer.append("<a href=\"activities/getAllActivities.action?number=0\">首页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if ((Integer.parseInt(this.number) + 1) == 1) {
			buffer.append("上一页");
		} else {
			buffer.append("<a href=\"activities/getAllActivities.action?number=" + (Integer.parseInt(this.number) - 1)
					+ "\">上一页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		...
		if (this.maxPage <= (Integer.parseInt(this.number) + 1)) {
			...
			buffer.append("<a href=\"activities/getAllActivities.action?number=" + (this.maxPage - 1) + "\">尾页</a>");
		}
	...
	public String getActivitiesById() {
		this.activities = this.activitiesBusiness.checkId(this.id);
		return SUCCESS;
	}

	// 按条件查询数据(模糊查询)
	public String queryActivitiesByCond() {
		this.list = new ArrayList<Activities>();
		if ("title".equals(this.cond)) {
			list = this.activitiesBusiness.checkByLikeTitle(this.name);
		}
		if ("contents".equals(this.cond)) {
			list = this.activitiesBusiness.checkByLikeContents(this.name);
		}
		if ("addtime".equals(this.cond)) {
			list = this.activitiesBusiness.checkByLikeAddtime(this.name);
		}
		if ("hits".equals(this.cond)) {
			list = this.activitiesBusiness.checkByLikeHits(this.name);
		}
		if ("num".equals(this.cond)) {
			list = this.activitiesBusiness.checkByLikeNum(this.name);
		}
		return SUCCESS;
	}
